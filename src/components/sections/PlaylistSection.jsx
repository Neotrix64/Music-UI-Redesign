import { useState, useEffect } from "react";
import usePlaylistStore from "../../store/usePlaylistStore";
import { getRelativeTime } from "../../utils/formatDate";

import UniversalTable from "../tables/AlbumTable";

function PlaylistSection() {
  const selectedPlaylist = usePlaylistStore((state) => state.selectedPlaylist);
  const setSelectedPlaylist = usePlaylistStore((state) => state.setSelectedPlaylist);
  const [isLoading, setIsLoading] = useState(false);

  // Efecto para manejar el loading cuando cambia la playlist seleccionada
  useEffect(() => {
    if (selectedPlaylist) {
      setIsLoading(false);
    }
  }, [selectedPlaylist]);

  // Función para simular carga (puedes reemplazar esto con tu lógica real)
  const handlePlaylistChange = (playlist) => {
    setIsLoading(true);
    setSelectedPlaylist(playlist);
    // La carga se completará cuando selectedPlaylist cambie
  };

  // Componente de loading
  const LoadingComponent = () => (
    <div className="text-white bg-[#181818] w-full h-[88vh] rounded-md mr-3 flex items-center justify-center">
      <div className="flex flex-col items-center gap-4">
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-white"></div>
        <p className="text-white/70">Loading playlist...</p>
      </div>
    </div>
  );

  // Componente cuando no hay playlist seleccionada
  const NoPlaylistComponent = () => (
    <div className="text-white bg-[#181818] w-full h-[88vh] rounded-md mr-3 flex items-center justify-center">
      <div className="text-center">
        <h3 className="text-2xl font-bold mb-2">No playlist selected</h3>
        <p className="text-white/70">Select a playlist to view its details</p>
      </div>
    </div>
  );

  // Mostrar loading si está cargando
  if (isLoading) {
    return <LoadingComponent />;
  }

  // Mostrar mensaje si no hay playlist seleccionada
  if (!selectedPlaylist) {
    return <NoPlaylistComponent />;
  }

  return (
    <div className="text-white bg-[#181818] w-full h-[88vh] rounded-md mr-3 overflow-y-scroll scrollbar-thumb-white/40 scrollbar">
      <div className="relative w-full">
        <div className="banner-container relative w-full h-[500px]">
          <img
            src={selectedPlaylist.playlistBanner || selectedPlaylist.albumCover || "NoBanner"}
            alt="playlist banner"
            className="absolute w-full h-full object-cover"
          />

          <div className="absolute inset-0 bg-gradient-to-b from-black/80 via-black/60 to-[#0d0d0d]"></div>

          <div className="absolute bottom-0 left-0 right-0 h-24 bg-gradient-to-t from-[#0d0d0d] to-transparent"></div>

          <div className="absolute bottom-12 left-6 right-6 z-10">
            <div className="flex flex-col max-w-2xl">
              <div className="mb-2">
                <span className="text-sm font-medium uppercase tracking-wider text-white/80">
                  Playlist
                </span>
              </div>
              <h2 className="text-6xl font-bold tracking-tight mb-2">
                {selectedPlaylist.name}
              </h2>
              <div className="flex items-center text-white/80 mt-2 mb-4">
                <img
                  src="Aqui foto"
                  alt="User"
                  className="h-6 w-6 rounded-full mr-2"
                />
                <span className="font-medium hover:text-white transition-colors duration-200 cursor-pointer">
                  {selectedPlaylist.idUser?.name || "User"}
                </span>
                <span className="mx-2">•</span>
                <span>{getRelativeTime(selectedPlaylist.creationDate)}</span>
                <span className="mx-2">•</span>
                <span>
                  {selectedPlaylist.idSongs?.length || 0} songs
                </span>
              </div>

              {/* Descripción de la playlist */}
              {selectedPlaylist.description && (
                <p className="text-white/80 mb-4 max-w-lg">
                  {selectedPlaylist.description}
                </p>
              )}

              {/* Controles */}
              <div className="flex items-center gap-4 mt-2">
                <button className="bg-green-500 hover:bg-green-400 text-white px-8 py-3 rounded-full font-medium flex items-center gap-2 transition-colors duration-300">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className="h-6 w-6"
                    viewBox="0 0 20 20"
                    fill="currentColor"
                  >
                    <path
                      fillRule="evenodd"
                      d="M10 18a8 8 0 100-16 8 8 0 000 16zM9.555 7.168A1 1 0 008 8v4a1 1 0 001.555.832l3-2a1 1 0 000-1.664l-3-2z"
                      clipRule="evenodd"
                    />
                  </svg>
                  Play
                </button>
                <button className="text-white border border-white/30 hover:border-white/70 p-3 rounded-full transition-colors duration-300">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className="h-6 w-6"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      strokeWidth={1.5}
                      d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"
                    />
                  </svg>
                </button>
                <button className="text-white hover:text-white/70 p-3 transition-colors duration-300">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className="h-6 w-6"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      strokeWidth={1.5}
                      d="M12 5v.01M12 12v.01M12 19v.01M12 6a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2z"
                    />
                  </svg>
                </button>
                <button className="text-white hover:text-white/70 p-3 transition-colors duration-300">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className="h-6 w-6"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      strokeWidth={1.5}
                      d="M12 4v16m8-8H4"
                    />
                  </svg>
                </button>
              </div>
            </div>
          </div>
        </div>

        <div className="content-section bg-gradient-to-b from-[#0d0d0d] to-[#0d0d0d] min-h-[500px] px-6 pb-96">
          <UniversalTable />

          <div className="flex gap-10">
            <div className="tracks-list space-y-2 w-[40%]">
              <h3 className="text-2xl font-bold mb-4 mt-14">Recommended</h3>
              <p className="text-white/70 mb-6">
                Based on what's in this playlist
              </p>
              
              {/* Aquí puedes agregar canciones recomendadas */}
              <div className="space-y-2">
                <div className="track-item flex items-center p-2 hover:bg-white/5 rounded-md cursor-pointer">
                  <img 
                    src="/api/placeholder/40/40" 
                    alt="Recommended song" 
                    className="h-10 w-10 rounded mr-3"
                  />
                  <div className="flex-1">
                    <div className="font-medium">Recommended Song 1</div>
                    <div className="text-sm text-white/50">Artist Name</div>
                  </div>
                  <button className="text-white/70 hover:text-white p-2">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      className="h-5 w-5"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                    >
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        strokeWidth={2}
                        d="M12 6v6m0 0v6m0-6h6m-6 0H6"
                      />
                    </svg>
                  </button>
                </div>
              </div>
            </div>

            <div className="flex-1">
              <h3 className="text-2xl font-bold mb-4 mt-14">About</h3>
              <div className="text-white/70">
                <p className="mb-4">
                  Created by {selectedPlaylist.idUser?.name || "NeoDev"}
                </p>
                <p>
                  Created {getRelativeTime(selectedPlaylist.creationDate)}
                </p>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

export default PlaylistSection;